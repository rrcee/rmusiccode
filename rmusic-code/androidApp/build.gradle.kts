...
sentry {
    ...
    if (isFullBuild) {
        ...
        // Disable auto-upload of proguard mapping on Windows to prevent build failures due to CLI path escaping issues on paths with spaces
        val isWindows = System.getProperty("os.name").lowercase().contains("win")
        autoUploadProguardMapping.set(!isWindows)
    } else {
...
tasks.configureEach {
    if (name.contains("uploadSentryProguardMappings")) {
        enabled = false
    }
}