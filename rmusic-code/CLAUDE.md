...
- Always ensure standard shell execution via `Bash` (e.g., `find`, `grep`, `rg`) if `glob` or `grep` fail schema validation.
- If calling `glob` or `grep`, strictly pass the `"pattern"` parameter as a string.
### Basic Information
