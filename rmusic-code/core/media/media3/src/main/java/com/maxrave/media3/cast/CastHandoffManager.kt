...
internal class CastHandoffManager(
    ...
    /** Resolves a WebVTT URL for synced-lyrics captions on the receiver (null = no lyrics). */
    private val lyricsVttUrlFor: (mediaId: String) -> String? = { null },
) {
    ...
    private fun GenericMediaItem.toCastMediaItem(stream: CastStreamResolver.ResolvedStream): MediaItem {
        ...
        // Synced lyrics ride to the TV as a WebVTT text track served from this device over the
        // LAN — the Default Media Receiver renders default tracks as timed captions, which is
        // how lyric lines appear alongside the artwork/progress UI.
        lyricsVttUrlFor(mediaId)?.let { vttUrl ->
            builder.setSubtitleConfigurations(
                listOf(
                    MediaItem.SubtitleConfiguration
                        .Builder(
                            android.net.Uri.parse(vttUrl),
