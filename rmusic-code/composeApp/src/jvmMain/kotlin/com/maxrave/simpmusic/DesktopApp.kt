...
import com.simpmusic.media_jvm.mpv.MpvPlayerEventAdapter
...
fun runDesktopApp(args: Array<String> = emptyArray()) {
    ...
    if (BuildKonfig.sentryDsn.isNotEmpty()) {
        Sentry.init { options ->
            ...
            options.release = "rmusic-desktop@${VersionManager.getVersionName()}"
    ...
    application {
        ...
        Window(
            ...
            Column(
                ...
                App(splashOverlay = { onFinished -> VideoSplashOverlay(onFinished) })
                ToastHost()
            }
        }
...
fun VideoSplashOverlay(onFinished: () -> Unit) {
    ...
    LaunchedEffect(Unit) {
        kotlinx.coroutines.delay(6500) // 6.5 seconds timeout failsafe
        safeOnFinished()
    }
    
    DisposableEffect(Unit) {
        val path = "C:\\Users\\Rishi\\Downloads\\splash screen.mp4"
        val mPlayer = MpvPlayer.create(audioOnly = false)
        if (mPlayer != null) {
            mPlayer.setEventListener(object : MpvPlayerEventAdapter() {
                override fun finished(p: MpvPlayer) {
                    safeOnFinished()
                }
                override fun error(p: MpvPlayer) {
                    safeOnFinished() // fallback
                }
            })
            mPlayer.loadFile(path, false)
            mPlayer.play()
            player = mPlayer
        } else {
            safeOnFinished() // fallback if mpv is not loaded
        }
    ...
    Box(
        ...
        val currentFrames = player?.videoFrames
        if (currentFrames != null) {
            var frame by remember(currentFrames) { mutableStateOf<ImageBitmap?>(null) }
            LaunchedEffect(currentFrames) {
                withContext(Dispatchers.Default) {
                    currentFrames.frames.collect { image ->
            ...
            Box(
                Modifier
                    .fillMaxSize()
                    .background(Color.Black)
                    .onSizeChanged { currentFrames.setTargetSize(it.width, it.height) },
                contentAlignment = Alignment.Center
            ) {
                frame?.let {
                    Image(
