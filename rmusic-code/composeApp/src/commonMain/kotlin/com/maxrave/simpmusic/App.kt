...
@Composable
fun RMusicSplashOverlay(onFinished: () -> Unit) {
    val letters = remember { "r/music".toList() }
    
    // Icon scale, alpha, and shine animations
    ...
    val shineProgress = remember { androidx.compose.animation.core.Animatable(-0.5f) }
    
    // Text and partition animations
    val letterProgress = remember { androidx.compose.animation.core.Animatable(0f) }
    val dividerProgress = remember { androidx.compose.animation.core.Animatable(0f) }
    val subtitleProgress = remember { androidx.compose.animation.core.Animatable(0f) }

    LaunchedEffect(Unit) {
        // Step 1: Icon entrance spring
        launch {
            iconAlpha.animateTo(
                1f,
                androidx.compose.animation.core.tween(500, easing = androidx.compose.animation.core.EaseOutQuad)
            )
        }
        launch {
            iconScale.animateTo(
        ...
        // Step 2: Shine swipe loop
        launch {
            kotlinx.coroutines.delay(500)
            shineProgress.animateTo(
                1.5f,
                androidx.compose.animation.core.tween(1200, easing = androidx.compose.animation.core.EaseInOutCubic)
            )
        }

        // Step 3: Text elements assembly
        ...
        launch {
            subtitleProgress.animateTo(
                1f,
                androidx.compose.animation.core.tween(600, easing = androidx.compose.animation.core.EaseOutQuad)
            )
        }

        // Total duration delay before finishing
    ...
    Box(
        modifier = Modifier
            .fillMaxSize()
            .background(Color.Black), // Premium black background
        ...
        Column(
            ...
            // New beautiful shimmering glass app icon
            Image(
                ...
                modifier = Modifier
                    ...
                    .drawWithContent {
                        drawContent()
                        if (shineProgress.value in -0.2f..1.2f) {
                            val shineWidth = 60.dp.toPx()
                            val x = size.width * shineProgress.value
                            drawRect(
            ...
            // Elegant large r/music typography, letter by letter
            Row {
                letters.forEachIndexed { index, char ->
                    val window = 0.35f
                    val startFraction = index * ((1f - window) / letters.size)
                    val raw = ((letterProgress.value - startFraction) / window).coerceIn(0f, 1f)
                    val eased = androidx.compose.animation.core.FastOutSlowInEasing.transform(raw)
                    Text(
                        text = char.toString(),
                        color = Color.White,
                        fontSize = 44.sp,
                        ...
                        modifier = Modifier.graphicsLayer {
                            alpha = eased
                            translationY = (1f - eased) * 16.dp.toPx()
                        }
                    )
                }
            }
            Spacer(modifier = Modifier.height(14.dp))
            // Hairline divider sweeping open between title and subtitle
            Box(
                modifier = Modifier
                    .width((140.dp.value * dividerProgress.value).dp)
                    .height(1.dp)
                    .clip(RoundedCornerShape(1.dp))
                    .background(Color.White.copy(alpha = 0.25f))
            )
            Spacer(modifier = Modifier.height(14.dp))
            // Secondary subtitle "Fork of SimpMusic"
            Text(
                text = "Fork of SimpMusic",
                color = Color.White,
                fontSize = 15.sp,
                fontWeight = FontWeight.Normal,
                fontFamily = androidx.compose.ui.text.font.FontFamily.SansSerif,
                modifier = Modifier.graphicsLayer {
                    alpha = subtitleProgress.value * 0.6f
                    translationY = (1f - subtitleProgress.value) * 8.dp.toPx()
                }
            )
        }
    }
...
fun App(
    ...
    splashOverlay: @Composable ((onFinished: () -> Unit) -> Unit)? = null
) {
    val windowSize = currentWindowAdaptiveInfo().windowSizeClass
    val navController = rememberNavController()
    val isDesktopShell = getPlatform() == Platform.Desktop

    val actualSplashOverlay = splashOverlay ?: { onFinished -> RMusicSplashOverlay(onFinished) }
    var isSplashShowing by remember { mutableStateOf(true) }
    var splashAlpha by remember { mutableStateOf(1f) }
    var uiAlpha by remember { mutableStateOf(0f) }
    var uiScale by remember { mutableStateOf(0.9f) }
    var uiTranslationY by remember { mutableStateOf(50f) }

    val animatedSplashAlpha by androidx.compose.animation.core.animateFloatAsState(
        targetValue = splashAlpha,
        animationSpec = androidx.compose.animation.core.tween(durationMillis = 600, easing = androidx.compose.animation.core.EaseOutQuad)
    )
    val animatedUiAlpha by androidx.compose.animation.core.animateFloatAsState(
        targetValue = uiAlpha,
        animationSpec = androidx.compose.animation.core.tween(durationMillis = 650, easing = androidx.compose.animation.core.EaseOutQuad)
    )
    val animatedUiScale by androidx.compose.animation.core.animateFloatAsState(
        targetValue = uiScale,
    ...
    val animatedUiTranslationY by androidx.compose.animation.core.animateFloatAsState(
        targetValue = uiTranslationY,
        animationSpec = androidx.compose.animation.core.spring(
            dampingRatio = androidx.compose.animation.core.Spring.DampingRatioLowBouncy,
            stiffness = androidx.compose.animation.core.Spring.StiffnessMediumLow
        )
    ...
    val isLiquidGlassEnabled by viewModel.getEnableLiquidGlass().collectAsStateWithLifecycle(DataStoreManager.TRUE)
    val dumEMode by viewModel.getDumEMode().collectAsStateWithLifecycle(DataStoreManager.FALSE)
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        Box(
            modifier = Modifier
                .fillMaxSize()
                .graphicsLayer {
                    alpha = animatedUiAlpha
                    scaleX = animatedUiScale
                    scaleY = animatedUiScale
                    translationY = animatedUiTranslationY * density
                }
        ) {
            AppTheme(
                themeMode = themeMode,
        ...
        if (dumEMode == TRUE) {
            com.maxrave.simpmusic.ui.screen.dume.DumEShell(
